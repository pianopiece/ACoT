...
def eval_model(args):
    ...
    # tasks = ['position']
    try:
        with open('./cache_dic1_mme_qwen2vl.json','r') as f:
            cache_dic1 = json.load(f)
        with open('./cache_dic2_mme_qwen2vl.json','r') as f:
            cache_dic2 = json.load(f)
        with open('./cache_dic3_mme_qwen2vl.json','r') as f:
    ...
    answers_path = f'data/MME_Benchmark/eval_tool/Qwen2vl_Results_{args.seed}'
    
    qs__prompts={'existence':[f'What objects are in the image?',
                            f'What attributes do objects have in the image?',
                            f"What are the relationships between objects?"],
                 
                 'count':[f'What objects are in the image?',
                            f'What attributes do objects have in the image?',
                            f"What are the relationships between objects?"],
                 
                 'position':[f'What objects are in the image?',
                            f'What attributes do objects have in the image?',
                            f"What are the relationships between objects?"],
                 
                 'color':[f'What objects are in the image?',
                            f'What attributes do objects have in the image?',
                            f"What are the relationships between objects?"]}
    ...
    with open('cache_dic1_qwen2vl.json', 'w') as file:
        json_data = json.dumps(cache_dic1)
        file.write(json_data)
    with open('cache_dic2_qwen2vl.json', 'w') as file:
        json_data = json.dumps(cache_dic2)
        file.write(json_data)
    with open('cache_dic3_qwen2vl.json', 'w') as file:
