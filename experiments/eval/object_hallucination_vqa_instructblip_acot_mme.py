...
def eval_model(args):
    ...
    try:
        with open('./cache_dic1_blip_mme.json','r') as f:
            cache_dic1 = json.load(f)
        with open('./cache_dic2_blip_mme.json','r') as f:
            cache_dic2 = json.load(f)
        with open('./cache_dic3_blip_mme.json','r') as f:
    ...
    answers_path = f'data/MME_Benchmark/eval_tool/InstructBLIP_Results_{args.seed}'
    for task in tasks:
        ...
        for question in tqdm(questions):
            ...
            image = Image.open(os.path.join(os.path.join(args.image_folder, task), image_file)).convert("RGB")
            ...
            print(outputs)
        ans_file.close()
    with open('cache_dic1_blip_mme.json', 'w') as file:
        json_data = json.dumps(cache_dic1)
        file.write(json_data)
    with open('cache_dic2_blip_mme.json', 'w') as file:
        json_data = json.dumps(cache_dic2)
        file.write(json_data)
    with open('cache_dic3_blip_mme.json', 'w') as file:
